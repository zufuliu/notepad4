...
public:
	...
	bool Contains(const PRectangle &rc) const {
		return (rc.left >= left) && (rc.right <= right) &&
			(rc.top >= top) && (rc.bottom <= bottom);
	}
	bool Intersects(const PRectangle &other) const {
...
public:
	...
	virtual void Polygon(const Point *pts, int npts, const ColourDesired &fore, const ColourDesired &back)=0;
	...
	virtual void DrawTextNoClip(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, const ColourDesired &fore, const ColourDesired &back)=0;
	virtual void DrawTextClipped(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, const ColourDesired &fore, const ColourDesired &back)=0;
	...
	virtual XYPOSITION ExternalLeading(const Font &font_)=0;
