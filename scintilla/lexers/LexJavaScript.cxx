...
void ColouriseJsDoc(Sci_PositionU startPos, Sci_Position lengthDoc, int initStyle, LexerWordList keywordLists, Accessor &styler) {
	...
	const int option = styler.GetPropertyInt("lexer.lang");
	...
	while (sc.More()) {
		...
		case SCE_JS_COMMENTBLOCKDOC:
			if (sc.atLineStart && (sc.state == SCE_JS_COMMENTLINE || sc.state == SCE_JS_COMMENTLINEDOC)) {
				sc.SetState(SCE_JS_DEFAULT);
				break;
			}
			switch (docTagState) {
			...
			if ((sc.state == SCE_JS_COMMENTBLOCK || sc.state == SCE_JS_COMMENTBLOCKDOC) && sc.Match('*', '/')) {
				sc.Forward();
		...
		if (sc.state == SCE_JS_DEFAULT) {
			if (sc.ch == '/') {
				if (sc.chNext == '/' || sc.chNext == '*') {
					...
					if (sc.ch == '!' || (sc.ch == chNext && sc.chNext != chNext)) {
						static_assert(SCE_JS_COMMENTLINEDOC - SCE_JS_COMMENTLINE == SCE_JS_COMMENTBLOCKDOC - SCE_JS_COMMENTBLOCK);
						sc.ChangeState(sc.state + SCE_JS_COMMENTLINEDOC - SCE_JS_COMMENTLINE);
					}
					if (chNext == '/') {
						if (visibleChars == 0) {
							lineStateLineType = JsLineStateMaskLineComment;
			...
			} else if (sc.ch == '<' && (option & JsOptionEnableJSX) != 0) {
				// <tag></tag>
				if (sc.chNext == '/') {
					--jsxTagLevel;
					sc.SetState(SCE_JSX_TAG);
