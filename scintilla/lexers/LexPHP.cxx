...
struct PHPLexer {
	...
	bool HighlightOperator(HtmlTextBlock block);
...
bool PHPLexer::HighlightOperator(HtmlTextBlock block) {
	kwType = KeywordType::None;
	if (block == HtmlTextBlock::PHP) {
...
void ColourisePHPDoc(Sci_PositionU startPos, Sci_Position lengthDoc, int initStyle, LexerWordList keywordLists, Accessor &styler) {
	...
	while (sc.More()) {
		...
		case css_style(SCE_CSS_COMMENT):
			if (sc.Match('*', '/')) {
				sc.Forward();
		...
		case css_style(SCE_CSS_SINGLESTRING):
		case css_style(SCE_CSS_DOUBLESTRING):
			if (sc.ch == '\\') {
				sc.Forward();
			} else if (sc.ch == ((sc.state == css_style(SCE_CSS_SINGLESTRING)) ? '\'' : '\"')) {
				sc.ForwardSetState(css_style(SCE_CSS_DEFAULT));
			}
			break;
		}
		...
		case js_style(SCE_JS_DEFAULT):
			if (sc.Match('<', '/') && lexer.HandleBlockEnd(HtmlTextBlock::Script)) {
				...
				if (lexer.HighlightOperator(HtmlTextBlock::Script)) {
					continue;
				}
			}
			break;
		...
		case css_style(SCE_CSS_DEFAULT):
			if (sc.Match('<', '/') && lexer.HandleBlockEnd(HtmlTextBlock::Style)) {
				...
				sc.SetState(css_style(SCE_CSS_COMMENT));
				sc.Forward();
			} else if (sc.ch == '\'') {
				sc.SetState(css_style(SCE_CSS_SINGLESTRING));
			} else if (sc.ch == '\"') {
				sc.SetState(css_style(SCE_CSS_DOUBLESTRING));
