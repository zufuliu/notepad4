...
int NestingLevelLookBehind(LexAccessor &styler, Sci_PositionU startPos) noexcept {
	int nestingLevel = 0;
...
void ColouriseCssDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, LexerWordList keywordLists, Accessor &styler) {
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (IsAWordChar(sc.chPrev) && (
			...
			case SCE_CSS_EXTENDED_IDENTIFIER:
			case SCE_CSS_UNKNOWN_IDENTIFIER:
				if (keywordLists[KeywordIndex_Property]->InList(s2))
			...
			case SCE_CSS_EXTENDED_PSEUDOELEMENT:
			case SCE_CSS_UNKNOWN_PSEUDOCLASS:
				if (op == ':' && opPrev != ':' && keywordLists[KeywordIndex_PseudoClass]->InListPrefixed(s2, '('))
					sc.ChangeState(SCE_CSS_PSEUDOCLASS);
				else if (opPrev == ':' && keywordLists[KeywordIndex_PseudoElement]->InListPrefixed(s2, '('))
...
void FoldCSSDoc(Sci_PositionU startPos, Sci_Position length, int, LexerWordList, Accessor &styler) {
	const Sci_PositionU endPos = startPos + length;
