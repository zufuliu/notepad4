...
enum {
	...
	KotlinLineStateMaskInterpolation = 1 << 2,
};
...
void ColouriseKotlinDoc(Sci_PositionU startPos, Sci_Position lengthDoc, int initStyle, LexerWordList keywordLists, Accessor &styler) {
	...
	if (startPos != 0) {
		...
		BacktrackToStart(styler, KotlinLineStateMaskInterpolation, startPos, lengthDoc, initStyle);
	}
	...
	if (sc.currentLine > 0) {
		...
		/*
		2: lineStateLineType
		1: KotlinLineStateMaskInterpolation
		1: unused
		8: commentLevel
		: stringInterpolatorCount
		*/
		commentLevel = (lineState >> 4) & 0xff;
		stringInterpolatorCount = lineState >> 12;
	...
	while (sc.More()) {
		...
		case SCE_KOTLIN_RAWSTRING:
			if (sc.atLineStart && sc.state != SCE_KOTLIN_RAWSTRING) {
				...
				if (sc.ch == '$') {
					const auto [interpolatorCount, chNext] = GetMatchedDelimiterCountEx(styler, sc.currentPos, '$');
					if (interpolatorCount >= stringInterpolatorCount && (chNext == '{' || IsIdentifierStartEx(chNext))) {
						escSeq.outerState = sc.state;
						sc.Advance(interpolatorCount - stringInterpolatorCount); // outer content
						sc.SetState(SCE_KOTLIN_OPERATOR2);
						sc.Advance(stringInterpolatorCount); // inner interpolation
						if (chNext == '{') {
							nestedState.push_back({escSeq.outerState, 1, stringInterpolatorCount});
						} else {
							sc.SetState(SCE_KOTLIN_IDENTIFIER);
						}
					} else {
						sc.Advance(interpolatorCount - 1);
					}
		...
		if (sc.state == SCE_KOTLIN_DEFAULT) {
			...
			if (sc.ch == '\"') {
				...
				if (chNext == '\"') {
					...
					sc.SetState(SCE_KOTLIN_STRING);
					sc.Advance(stringInterpolatorCount);
					if (sc.MatchNext('"', '"')) {
						sc.ChangeState(SCE_KOTLIN_RAWSTRING);
						sc.Forward(2);
					}
		...
		if (sc.atLineEnd) {
			int lineState = (commentLevel << 4) | lineStateLineType | (stringInterpolatorCount << 12);
			if (!nestedState.empty()) {
				lineState |= KotlinLineStateMaskInterpolation;
			}
