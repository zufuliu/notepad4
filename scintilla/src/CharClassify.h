...
// Created with Python 3.14.0a5, Unicode 16.0.0
	static CharacterClass ClassifyCharacter(uint32_t ch) noexcept {
		...
		ch = (CharClassifyTable[(ch >> 3) + 1600] << 3) | (ch & 7);
		return static_cast<CharacterClass>(CharClassifyTable[ch + 3208]);
	}
...
//grapheme function++Autogenerated -- start of section automatically generated
	static GraphemeBreakProperty GetGraphemeBreakProperty(uint32_t ch) noexcept {
		...
		ch = (GraphemeBreakTable[ch >> 12] << 9) | (ch & 4095);
		ch = (GraphemeBreakTable[(ch >> 6) + 225] << 6) | (ch & 63);
		ch = (GraphemeBreakTable[(ch >> 3) + 1505] << 3) | (ch & 7);
		return static_cast<GraphemeBreakProperty>(GraphemeBreakTable[ch + 3337]);
	}
