...
void Indicator::Draw(Surface *surface, PRectangle rc, PRectangle rcLine, PRectangle rcCharacter, State state, int value) const {
	...
	case IndicatorStyle::Strike: {
		...
		const PRectangle rcStrike(rcAligned.left, yStrike, rcAligned.right, yStrike + strokeWidth);
	...
	case IndicatorStyle::Dash: {
		...
		const XYPOSITION width = std::round(strokeWidth);
		const XYPOSITION widthDash = 3 + width;
		while (x < rc.right) {
			const PRectangle rcDash = PRectangle(x, ymid, x + widthDash, ymid + width);
	...
	case IndicatorStyle::Dots: {
		...
		XYPOSITION x = std::floor(rc.left);
		while (x < rc.right) {
			const PRectangle rcDot = PRectangle(x, ymid,x + widthDot, ymid + widthDot);
