...
int SCI_METHOD Document::GetLineIndentation(int line) const {
	int indent = 0;
	if ((line >= 0) && (line < LinesTotal())) {
...
void SCI_METHOD Document::StartStyling(int position, unsigned char) {
	endStyled = position;
}

bool SCI_METHOD Document::SetStyleFor(int length, unsigned char style) {
	if (enteredStyling != 0) {
		return false;
	} else {
		enteredStyling++;
...
bool SCI_METHOD Document::SetStyles(int length, const unsigned char *styles) {
	if (enteredStyling != 0) {
		return false;
	} else {
		enteredStyling++;
...
bool Document::IsWordPartSeparator(char ch) const {
	return (WordCharClass(ch) == CharClassify::ccWord || WordCharClass(ch) == CharClassify::ccUniWord) && IsPunctuation(ch);
}
