...
void Document::DeleteMarkFromHandle(int markerHandle) {
	...
	DocModification mh(SC_MOD_CHANGEMARKER, 0, 0, 0, nullptr);
	mh.line = -1;
	NotifyModified(mh);
}
	...
	if (dbcsCodePage) {
		...
		if (SC_CP_UTF8 == dbcsCodePage) {
			...
			if (IsDBCSLeadByte(leadByte)) {
				bytesInCharacter = 2;
				character = (leadByte << 8) | cb.CharAt(position + 1);
			} else {
				character = leadByte;
			}
		}
...
#if WCHAR_T_IS_16
			const size_t outLen = UTF16FromUTF8(sv, ws.data(), lenS);
#else
			const size_t outLen = UTF32FromUTF8(sv, reinterpret_cast<unsigned int *>(ws.data()), lenS);
#endif
			ws[outLen] = 0;
			std::wregex regexp;
			regexp.assign(ws.data(), flagsRe);
