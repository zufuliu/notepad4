...
constexpr KeyMod KeyModFromWParam(uptr_t x) noexcept {
	constexpr uptr_t shiftForKeyMod = 16;
	return static_cast<KeyMod>(x >> shiftForKeyMod);
}

constexpr Keys KeysFromWParam(uptr_t x) noexcept {
...
int Editor::DelWordOrLine(Message iMessage) {
	...
	for (size_t r = 0; r < sel.Count(); r++) {
		...
		case Message::DelWordRightEnd:
			...
			break;
		case Message::DelLineLeft:
			rangeDelete = Range(pdoc->LineStartPosition(caretPosition), caretPosition);
			break;
		case Message::DelLineRight:
			rangeDelete = Range(caretPosition, pdoc->LineEndPosition(caretPosition));
			break;
		default:
			break;
		}
...
bool Editor::CopyLineRange(SelectionText &ss, bool allowProtected) const {
	const Sci::Line currentLine = pdoc->SciLineFromPosition(sel.MainCaret());
...
void Editor::DropAt(SelectionPosition position, std::string_view value, bool moving, bool rectangular) {
	...
	if ((inDragDrop != DragDrop::dragging) || !positionWasInSelection ||
		...
		const std::string convertedText = Document::TransformLineEnds(value, pdoc->eolMode);
...
void Editor::DropSelection(size_t part) noexcept {
...
sptr_t Editor::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::ReplaceRectangular: {
		...
		InsertPasteShape(std::string_view{ConstCharPtrFromSPtr(lParam), wParam}, PasteShape::rectangular);
		break;
	}
	...
	case Message::BraceMatch: {
		...
		// const ElapsedPeriod period;
		const Sci::Position position = pdoc->BraceMatch(PositionFromUPtr(wParam), lParam);
		// const double duration = period.Duration()*1e3;
