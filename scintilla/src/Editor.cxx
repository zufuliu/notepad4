...
SelectionPosition Editor::ClampPositionIntoDocument(const SelectionPosition &sp) const {
	if (sp.Position() < 0) {
		...
		SelectionPosition sp_ = sp;
		if (!pdoc->IsLineEndPosition(sp.Position()))
			sp_.SetVirtualSpace(0);
		return sp_;
	}
}
...
SelectionPosition Editor::SPositionFromLocation(const Point &pt_, bool canReturnInvalid, bool charPosition, bool virtualSpace) {
	RefreshStyleData();
	AutoSurface surface(this);
	Point pt = pt_;
	if (canReturnInvalid) {
...
int Editor::LineFromLocation(const Point &pt) const {
...
PRectangle Editor::RectangleFromRange(const Range &r, int overlap) {
...
SelectionPosition Editor::MovePositionOutsideChar(const SelectionPosition &pos_, int moveDir, bool checkLineEnd) const {
...
int Editor::MovePositionTo(const SelectionPosition &newPos_, Selection::selTypes selt, bool ensureVisible) {
...
void Editor::PasteRectangular(const SelectionPosition &pos, const char *ptr, int len) {
	if (pdoc->IsReadOnly() || SelectionContainsProtected()) {
		return;
	}
	sel.Clear();
...
void Editor::NotifyDoubleClick(const Point &pt, bool shift, bool ctrl, bool alt) {
...
static bool Close(const Point &pt1, const Point &pt2) {
...
void Editor::ButtonMoveWithModifiers(const Point &pt, int modifiers) {
...
void Editor::ButtonMove(const Point &pt) {
	ButtonMoveWithModifiers(pt, 0);
}

void Editor::ButtonUp(const Point &pt, unsigned int curTime, bool ctrl) {
...
int Editor::PositionAfterArea(const PRectangle &rcArea) const {
...
bool Editor::PaintContains(const PRectangle &rc) {
	if (rc.Empty()) {
		return true;
	} else {
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_SETSTYLINGEX:             // Specify a complete styling buffer
		if (lParam == 0)
			return 0;
		pdoc->SetStyles(static_cast<int>(wParam), reinterpret_cast<const unsigned char*>(CharPtrFromSPtr(lParam)));
		break;
