...
protected:	// ScintillaBase subclass needs access to much of Editor
	...
	Point DocumentPointFromView(const Point &ptView) const;  // Convert a point from view space to document
	...
	Point LocationFromPosition(const SelectionPosition &pos);
	Point LocationFromPosition(int pos);
	int XFromPosition(int pos);
	int XFromPosition(const SelectionPosition &sp);
	SelectionPosition SPositionFromLocation(const Point &pt, bool canReturnInvalid=false, bool charPosition=false, bool virtualSpace=true);
	int PositionFromLocation(const Point &pt, bool canReturnInvalid=false, bool charPosition=false);
	SelectionPosition SPositionFromLineX(int lineDoc, int x);
	int PositionFromLineX(int line, int x);
	int LineFromLocation(const Point &pt) const;
	...
	void SetXYScroll(const XYScrollPosition &newXY);
	...
	void Paint(Surface *surfaceWindow, const PRectangle &frcArea);
	...
	void Cut();
	void PasteRectangular(const SelectionPosition &pos, const char *ptr, int len);
	virtual void Copy() = 0;
	...
	virtual void NotifyDoubleClick(const Point &pt, bool shift, bool ctrl, bool alt);
	...
	bool NotifyMarginClick(const Point &pt, bool shift, bool ctrl, bool alt);
	...
	void NotifyModified(Document *document, const DocModification &mh, void *userData);
	...
	void ButtonMoveWithModifiers(const Point &pt, int modifiers);
	void ButtonMove(const Point &pt);
	void ButtonUp(const Point &pt, unsigned int curTime, bool ctrl);
