...
public:
	...
	void PenColour(const ColourDesired &fore);
	int LogPixelsY() const;
	int DeviceHeightFont(int points) const;
	void MoveTo(int x_, int y_);
	void LineTo(int x_, int y_);
	void Polygon(const Point *pts, int npts, const ColourDesired &fore, const ColourDesired &back);
	void RectangleDraw(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	void FillRectangle(const PRectangle &rc, const ColourDesired &back);
	void FillRectangle(const PRectangle &rc, const Surface &surfacePattern);
	void RoundedRectangle(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	...
	void Ellipse(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	void Copy(const PRectangle &rc, const Point &from, const Surface &surfaceSource);

	void DrawTextCommon(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, UINT fuOptions);
	void DrawTextNoClip(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, const ColourDesired &fore, const ColourDesired &back);
	void DrawTextClipped(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, const ColourDesired &fore, const ColourDesired &back);
...
bool SurfaceGDI::Initialised() const {
	return hdc != 0;
}
...
int SurfaceGDI::LogPixelsY() const {
	return ::GetDeviceCaps(hdc, LOGPIXELSY);
}

int SurfaceGDI::DeviceHeightFont(int points) const {
...
void SurfaceGDI::Polygon(const Point *pts, int npts, const ColourDesired &fore, const ColourDesired &back) {
	PenColour(fore);
	BrushColor(back);
...
void SurfaceGDI::FillRectangle(const PRectangle &rc, const Surface &surfacePattern) {
	HBRUSH br;
...
void SurfaceGDI::Copy(const PRectangle &rc, const Point &from, const Surface &surfaceSource) {
	::BitBlt(hdc,
...
void SurfaceGDI::DrawTextNoClip(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
...
void SurfaceGDI::DrawTextClipped(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
	const ColourDesired &fore, const ColourDesired &back) {
	::SetTextColor(hdc, fore.AsLong());
	::SetBkColor(hdc, back.AsLong());
...
void SurfaceGDI::DrawTextTransparent(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
...
XYPOSITION SurfaceGDI::ExternalLeading(const Font &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
...
void SurfaceGDI::SetClip(const PRectangle &rc) {
...
public:
	...
	void MoveTo(int x_, int y_);
	void LineTo(int x_, int y_);
	void Polygon(Point *pts, int npts, const ColourDesired &fore, const ColourDesired &back);
	void RectangleDraw(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	void FillRectangle(const PRectangle &rc, const ColourDesired &back);
	void FillRectangle(const PRectangle &rc, Surface &surfacePattern);
	void RoundedRectangle(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	void AlphaRectangle(const PRectangle &rc, int cornerSize, const ColourDesired &fill, int alphaFill,
		const ColourDesired &outline, int alphaOutline, int flags);
	void DrawRGBAImage(const PRectangle &rc, int width, int height, const unsigned char *pixelsImage);
	void Ellipse(const PRectangle &rc, const ColourDesired &fore, const ColourDesired &back);
	void Copy(const PRectangle &rc, const Point &from, const Surface &surfaceSource);
...
bool SurfaceD2D::Initialised() const {
...
int SurfaceD2D::LogPixelsY() const {
	return logPixelsY;
}

int SurfaceD2D::DeviceHeightFont(int points) const {
	return ::MulDiv(points, LogPixelsY(), 72);
}
...
void SurfaceD2D::DrawTextCommon(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len, UINT fuOptions) {
	SetFont(font_);
...
void SurfaceD2D::DrawTextNoClip(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
	const ColourDesired &fore, const ColourDesired &back) {
	if (pRenderTarget) {
...
void SurfaceD2D::DrawTextClipped(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
	const ColourDesired &fore, const ColourDesired &back) {
	if (pRenderTarget) {
		FillRectangle(rc, back);
		D2DPenColour(fore);
...
void SurfaceD2D::DrawTextTransparent(const PRectangle &rc, const Font &font_, XYPOSITION ybase, const char *s, int len,
	const ColourDesired &fore) {
	// Avoid drawing spaces in transparent mode
	for (int i=0; i<len; i++) {
		if (s[i] != ' ') {
...
XYPOSITION SurfaceD2D::WidthText(const Font &font_, const char *s, int len) {
	FLOAT width = 1.0;
	SetFont(font_);
	const TextWide tbuf(s, len, unicodeMode, codePageText);
...
void SurfaceD2D::SetClip(const PRectangle &rc) {
	if (pRenderTarget) {
		D2D1_RECT_F rcClip = {rc.left, rc.top, rc.right, rc.bottom};
		pRenderTarget->PushAxisAlignedClip(rcClip, D2D1_ANTIALIAS_MODE_ALIASED);
		clipsActive++;
...
void Window::SetPositionRelative(PRectangle &rc, const Window &w) {
...
/* Returns rectangle of monitor pt is on, both rect and pt are in Window's
   coordinates */
PRectangle Window::GetMonitorRect(const Point &pt) const {
...
public:
	...
	virtual void Create(const Window &parent_, int ctrlID_, const Point &location_, int lineHeight_, bool unicodeMode_, int technology_);
	virtual void SetAverageCharWidth(int width);
	virtual void SetVisibleRows(int rows);
	virtual int GetVisibleRows() const;
	virtual PRectangle GetDesiredRect();
	virtual int CaretFromEdge() const;
	virtual void Clear();
	...
	virtual void GetValue(int n, char *value, int len) const;
...
void ListBoxX::Create(const Window &parent_, int ctrlID_, const Point &location_, int lineHeight_, bool unicodeMode_, int technology_) {
	parent = &parent_;
	ctrlID = ctrlID_;
	location = location_;
...
static inline XYPOSITION XYMinimum(XYPOSITION a, XYPOSITION b) {
	if (a < b)
		return a;
	else
		return b;
...
static inline XYPOSITION XYMaximum(XYPOSITION a, XYPOSITION b) {
	if (a > b)
		return a;
	else
		return b;
...
LRESULT ListBoxX::NcHitTest(WPARAM wParam, LPARAM lParam) const {
	...
	if (hit >= HTTOP && hit <= HTTOPRIGHT) {
		...
		const PRectangle &rc = const_cast<ListBoxX*>(this)->GetPosition();
...
POINT ListBoxX::GetClientExtent() const {
	const PRectangle &rc = const_cast<ListBoxX*>(this)->GetClientPosition();
	POINT ret;
...
void Menu::Show(const Point &pt, const Window &w) {
