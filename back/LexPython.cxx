...
static enum kwType { kwOther, kwClass, kwDef, kwImport, kwCDef, kwCTypeName, kwCPDef };
...
static enum literalsAllowed { litNone=0, litU=1, litB=2};
...
static void ColourisePyDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[], Accessor &styler) {
	...
	WordList &kwBuildins = *keywordlists[2];
	...
	if (startPos > 0) {
		if (lineCurrent > 0) {
			...
			while (lineCurrent > 0) {
				...
				if (eolStyle == SCE_P_STRING || eolStyle == SCE_P_CHARACTER || eolStyle == SCE_P_STRINGEOL) {
					lineCurrent -= 1;
				} else {
					break;
				}
	...
	const bool base2or8Literals = styler.GetPropertyInt("lexer.python.literals.binary", 1) != 0;
	...
	const bool stringsOverNewline = styler.GetPropertyInt("lexer.python.strings.over.newline") != 0;
	...
	int spaceFlags = 0;
	LexIndentAmount(styler, lineCurrent, &spaceFlags, IsPyComment);
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.atLineEnd) {
			if ((sc.state == SCE_P_DEFAULT) || (sc.state == SCE_P_TRIPLE) || (sc.state == SCE_P_TRIPLEDOUBLE)) {
		...
		if (sc.state == SCE_P_OPERATOR) {
			...
			if (!iswordchar(sc.ch) || sc.ch == '.') {
				...
				if ((kwLast == kwImport) && (strcmp(s, "as") == 0)) {
					...
					style = SCE_P_WORD;
				} else if (kwBuildins.InList(s)) {
					style = SCE_P_BUILDINS;
		...
		} else if (sc.state == SCE_P_COMMENTBLOCK2) {
			if (sc.Match('*', '/')) {
				sc.Forward(2);
				sc.SetState(SCE_P_DEFAULT);
			}
		...
		if (sc.state == SCE_P_DEFAULT) {
			...
			} else if (lexType == LEX_BOO && sc.Match('/', '/')) {
				sc.SetState(SCE_P_COMMENTLINE);
			} else if (lexType == LEX_BOO && sc.Match('/', '*')) {
				sc.SetState(SCE_P_COMMENTBLOCK2);
				sc.Forward();
...
#define IsCommentLine(line)		IsLexSingleCommentLine(line, styler, '#', SCE_P_COMMENTLINE, SCE_P_COMMENTBLOCK)
#define IsBooCommentLine(line)	IsLexDoubleCommentLine(line, styler, '/', '/', SCE_P_COMMENTLINE)
