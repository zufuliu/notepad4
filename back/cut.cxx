...
ColourDesired Editor::TextBackground(const ViewStyle &vsDraw,
        const ColourOptional& background, int inSelection, bool inHotspot, int styleMain, int i, LineLayout *ll) const {
	if (inSelection == 1) {
...
void Editor::DrawIndentGuide(Surface *surface, int lineVisible, int lineHeight, int start, const PRectangle &rcSegment, bool highlight) {
...
void Editor::DrawWrapMarker(Surface *surface, const PRectangle &rcPlace,
...
void DrawTextBlob(Surface *surface, const ViewStyle &vsDraw, const PRectangle &rcSegment,
				  const char *s, const ColourDesired &textBack, const ColourDesired &textFore, bool twoPhaseDraw) {
	if (!twoPhaseDraw) {
...
void Editor::DrawEOL(Surface *surface, const ViewStyle &vsDraw, const PRectangle &rcLine, LineLayout *ll,
...
void Editor::DrawIndicator(int indicNum, int startPos, int endPos, Surface *surface, ViewStyle &vsDraw,
		int xStart, const PRectangle &rcLine, LineLayout *ll, int subLine) {
...
void Editor::DrawIndicators(Surface *surface, ViewStyle &vsDraw, int line, int xStart,
        const PRectangle &rcLine, LineLayout *ll, int subLine, int lineEnd, bool under) {
	// Draw decorators
	const int posLineStart = pdoc->LineStart(line);
...
void Editor::DrawAnnotation(Surface *surface, const ViewStyle &vsDraw, int line, int xStart,
    const PRectangle &rcLine, LineLayout *ll, int subLine) {
...
void Editor::DrawBlockCaret(Surface *surface, const ViewStyle &vsDraw, LineLayout *ll, int subLine,
	int xStart, int offset, int posCaret, PRectangle &rcCaret, const ColourDesired &caretColour) const {
			...
			if (ll) {
				...
				if ((level & SC_FOLDLEVELHEADERFLAG) &&
					...
					if ((expanded && (foldFlags & SC_FOLDFLAG_LINEBEFORE_EXPANDED))
						...
						surface->FillRectangle(rcFoldLine, vs.markers[SC_MARKNUM_FOLDER].back);
					}
