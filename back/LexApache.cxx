...
static void ColouriseConfDoc(unsigned int startPos, int length, int, WordList *keywordLists[], Accessor &styler) {
	...
	WordList &directives_xml = *keywordLists[0];
	WordList &directives = *keywordLists[1];
	WordList &params = *keywordLists[2];
		...
		switch(state) {
			...
			case SCE_CONF_IDENTIFIER:
				...
				if( (IsASCII(ch) && isalnum(ch)) || (ch == '_') || (ch == '-') || (ch == '/') || (ch == '$') || (ch == '.') || (ch == '*')) {
					...
					if(directives_xml.InList(buffer) || directives.InList(buffer)) {
