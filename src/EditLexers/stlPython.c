...
// math/cmath
"ceil() copysign() fabs() factorial() floor() fmod() frexp() fsum() isfinite() isinf() isnan() ldexp() modf() trunc() exp() expm1() log() log1p() log2() log10() pow() sqrt() acos() asin() atan() atan2() cos() hypot() sin() tan() degrees() radians() acosh() asinh() atanh() cosh() sinh() tanh() erf() erfc() gamma() lgamma() " "conjugate() phase() polar() rect() "
// Fraction
"gcd() from_float() from_decimal() limit_denominator() "
// random
"seed() getstate() setstate() getrandbits() randrange() randint() choice() shuffle() sample() random() uniform() triangular() betavariate() expovariate() gammavariate() gauss() lognormvariate() normalvariate() vonmisesvariate() paretovariate() weibullvariate() "
// decimal
"getcontext() setcontext() localcontext() " "adjusted() as_tuple() canonical() compare() compare_signal() compare_total() compare_total_mag() conjugate() copy_abs() copy_negate() copy_sign() rom_float() fma() is_canonical() is_finite() is_infinite() is_nan() is_normal() is_qnan() is_signed() is_snan() is_subnormal() is_zero() ln() logb() logical_and() logical_invert() logical_or() logical_xor() max_mag()  min_mag() next_minus() next_plus() next_toward() normalize() number_class() quantize() radix() remainder_near() rotate() same_quantum() scaleb() shift() to_eng_string() to_integral() to_integral_exact() to_integral_value() " "clear_flags() clear_traps() copy_decimal() create_decimal() create_decimal_from_float() Etiny() Etop() add() divide() divide_int() minus() multiply() plus() power() remainder() "
// sys
...
EDITLEXER lexPython  = { SCLEX_PYTHON, NP2LEX_PYTHON, L"Python Script", L"py; pyw", L"", &Keywords_Python,
{
	...
	{ SCE_PY_DEFNAME, NP2STYLE_FunctionDef, L"Function Define", L"bold; fore:#0080C0", L"" },
	{ SCE_PY_FUNCTION, NP2STYLE_Function, L"Function", L"fore:#A46000", L"" },
	{ SCE_PY_ATTR, NP2STYLE_Attribute, L"Attribute", L"fore:#FF8000", L""},
