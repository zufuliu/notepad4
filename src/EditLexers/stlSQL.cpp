...
static const char * const Keywords_SQL[] = {
...
"audit authentication authorization authors auto_increment autoextend_size autoincrement availability avg avg_row_length "
...
"chain chaining challenge_response change changed channel char_length "
...
"delay_key_write delayed delete delete_domain_id delimiter delimiters dense_rank deny depends depth deref derived "
"des_key_file desc describe describe_catalog describe_name "
...
"end end-exec end_frame end_partition endpoint ends enforced engine engine_attribute engines equals errlvl error errors "
"escape escaped event events every except exception exchange exclude excluding exclusive exec execute exists exit "
"exp expansion expire explain export expression extended extension extent_size external extract "
"factor fail failed_login_attempts false family fast faults fetch fields "
"file file_block_size filelistonly fillfactor filter final finish finish_catalog finish_name "
...
"general generate generated get get_format get_master_public_key get_source_public_key get_transmission_status "
"glob global go goto governor grant granted grants greatest group group_replication grouping groups gtid_only "
...
"initial initial_size initialize initially initiate inline inner innobase innodb inout input "
...
"keep key key_block_size key_member key_type keyring keys kill "
...
"master_ssl_verify_server_cert master_tls_ciphersuites master_tls_version master_user master_zstd_compression_level "
...
"microsecond middleint migrate min min_rows minus minute_microsecond minute_second minvalue "
...
"pack_keys package pad page page_checksum parallel parameter parameter_mode parameter_name parameter_ordinal_position "
...
"regexp registration regr_avgx regr_avgy regr_count regr_intercept regr_r2 regr_slope regr_sxx regr_sxy regr_syy reindex "
...
"row_count row_format row_number rowcount rowguidcol rownum rows rowtype rtree rtrim rule running "
"save savepoint "
...
"slave slow snapshot socket some soname sounds source source_auto_position source_bind "
"source_compression_algorithms source_connect_retry source_delay source_heartbeat_period source_host "
"source_log_file source_log_pos source_password source_port source_public_key_path source_retry_count source_ssl "
"source_ssl_ca source_ssl_capath source_ssl_cert source_ssl_cipher source_ssl_crl source_ssl_crlpath source_ssl_key "
"source_ssl_verify_server_cert source_tls_ciphersuites source_tls_version source_user source_zstd_compression_level "
"space spatial specific specific_name specification specifictype sql sql_after_gtids sql_after_mts_gaps "
...
"sql_tsi_week sql_tsi_year sqlca sqlcode sqlerror sqlexception sqlstate sqlwarning sqrt srid ssl stable stacked "
"standalone start start_catalog start_name "
...
"succeeds sum super support suspend swaps switches symmetric synonym sysdate sysid system system_time system_user "
...
"unicode uninstall union unique unknown unlink unlisten unlock unlogged unmatched unnamed unnest unpivot unregister "
"until untyped update updatetext upgrade upper uri url usage use use_frm "
...
"can_access_column( can_access_database( can_access_table( can_access_user( can_access_view( centroid( changes( "
"coercibility( "
...
"field( find_in_set( format_bytes( format_pico_time( found_rows( from_base64( from_days( from_unixtime( "
...
"internal_data_free( internal_data_length( internal_dd_char_length( internal_get_comment_or_error( "
"internal_get_enabled_role_json( internal_get_hostname( internal_get_username( internal_get_view_warning_or_error( "
"internal_index_column_cardinality( internal_index_length( internal_is_mandatory_role( internal_keys_disabled( "
"internal_max_data_length( internal_table_rows( internal_update_time( intersects( "
...
"json_depth( json_detailed( json_each( json_equals( json_extract( json_group_array( json_group_object( json_insert( "
"json_keys( json_length( json_loose( json_merge( json_merge_patch( json_merge_preserve( json_normalize( json_overlaps( "
"json_patch( json_pretty( json_quote( json_remove( json_replace( "
"json_search( json_set( json_storage_free( json_storage_size( json_tree( json_type( json_unquote( json_valid( julianday( "
...
"printf( ps_current_thread_id( ps_thread_id( "
...
"APPLICATION ARE ARITHABORT ARITHIGNORE ARRAY_AGG ARRAY_MAX_CARDINALITY "
"AS ASC ASCII ASENSITIVE ASIN ASSEMBLY ASSERTION ASSIGNMENT ASYMMETRIC AT ATAN ATOMIC ATTACH ATTRIBUTE ATTRIBUTES "
"AUDIT AUTHENTICATION AUTHORIZATION AUTHORS AUTOEXTEND_SIZE AUTOINCREMENT AUTO_INCREMENT AVAILABILITY AVG AVG_ROW_LENGTH "
"BACKUP BACKWARD BASE64 BEFORE BEGIN BEGIN_FRAME BEGIN_PARTITION BERNOULLI BETWEEN BINDING BINLOG BIT_LENGTH "
"BLOCK BLOCKED BODY BOM BOTH BREADTH BREAK BROKER BROWSE BTREE BUCKETS BULK BY "
"CACHE CACHING CALL CALLED CARDINALITY CASCADE CASCADED CASE CAST CATALOG CATALOG_NAME CEIL CEILING CERTIFICATE "
"CHAIN CHAINING CHALLENGE_RESPONSE CHANGE CHANGED CHANNEL "
...
"DELAYED DELAY_KEY_WRITE DELETE DELETE_DOMAIN_ID DELIMITER DELIMITERS DENSE_RANK DENY DEPENDS DEPTH DEREF DERIVED DESC "
...
"DYNAMIC DYNAMIC_FUNCTION DYNAMIC_FUNCTION_CODE "
"EACH ELEMENT ELSE ELSEIF ELSIF EMPTY ENABLE ENCLOSED ENCODING ENCRYPTED ENCRYPTION "
"END END-EXEC ENDPOINT ENDS END_FRAME END_PARTITION ENFORCED ENGINE ENGINES ENGINE_ATTRIBUTE EQUALS ERRLVL ERROR ERRORS "
"ESCAPE ESCAPED EVENT EVENTS EVERY EXCEPT EXCEPTION EXCHANGE EXCLUDE EXCLUDING EXCLUSIVE EXEC EXECUTE EXISTS EXIT "
"EXP EXPANSION EXPIRE EXPLAIN EXPORT EXPRESSION EXTENDED EXTENSION EXTENT_SIZE EXTERNAL EXTRACT "
"FACTOR FAIL FAILED_LOGIN_ATTEMPTS FALSE FAMILY FAST FAULTS FETCH FIELDS "
"FILE FILELISTONLY FILE_BLOCK_SIZE FILLFACTOR FILTER FINAL FINISH FINISH_CATALOG FINISH_NAME "
"FINISH_PROCEDURE_SPECIFIC_CATALOG FINISH_PROCEDURE_SPECIFIC_NAME FINISH_PROCEDURE_SPECIFIC_SCHEMA FINISH_SCHEMA "
"FIPS_FLAGGER FIRST FIRST_VALUE FLAG FLOOR FLUSH FMTONLY "
"FOLLOWING FOLLOWS FOR FORCE FORCEPLAN FOREIGN FORMAT FORTRAN FORWARD FOUND "
"FRAC_SECOND FRAME_ROW FREE FREETEXT FREETEXTTABLE FREEZE FROM FS FULFILL FULFILL_CATALOG FULFILL_NAME "
...
"FULL FULLTEXT FULLTEXTTABLE FUNCTION FUNCTIONS FUSION "
"GENERAL GENERATE GENERATED GET GET_FORMAT GET_MASTER_PUBLIC_KEY GET_SOURCE_PUBLIC_KEY GET_TRANSMISSION_STATUS "
"GLOB GLOBAL GO GOTO GOVERNOR GRANT GRANTED GRANTS GREATEST GROUP GROUPING GROUPS GROUP_REPLICATION GTID_ONLY "
...
"IMMEDIATE IMMEDIATELY IMMUTABLE IMPLEMENTATION IMPLICIT IMPLICIT_TRANSACTIONS IMPORT IN INACTIVE "
"INCLUDE INCLUDING INCREMENT INDENT INDEX INDEXED INDEXES INDICATOR INFILE INHERIT INHERITS "
"INITIAL INITIALIZE INITIALLY INITIAL_SIZE INITIATE INLINE INNER INNOBASE INNODB INOUT INPUT "
"INSENSITIVE INSERT INSERT_METHOD INSTALL INSTANCE INSTANTIABLE INSTEAD INTEGRITY INTERSECT INTERSECTION INTO INVOKER "
"IO IO_AFTER_GTIDS IO_BEFORE_GTIDS IO_THREAD IPC IS ISNULL ISOLATION ISSUER IS_PRUNABLE ITERATE "
"JOIN "
"JSON_ARRAY JSON_ARRAYAGG JSON_EXISTS JSON_OBJECT JSON_OBJECTAGG JSON_QUERY JSON_TABLE JSON_TABLE_PRIMITIVE JSON_VALUE "
"KEEP KEY KEYRING KEYS KEY_BLOCK_SIZE KEY_MEMBER KEY_TYPE KILL "
"LABEL LABELONLY LAG LANGUAGE LAST LAST_VALUE LATERAL LEAD LEADING LEAKPROOF LEAST LEAVE LEAVES LEFT LENGTH LESS LEVEL "
"LIBRARY LIKE LIKE_REGEX LIMIT LINEAR LINENO LINES LINK LIST LISTAGG LISTEN LN LOAD "
...
"MEASURES MEDIUM MEMBER MEMORY MERGE MESSAGE MESSAGE_LENGTH MESSAGE_OCTET_LENGTH MESSAGE_TEXT METHOD "
"MICROSECOND MIDDLEINT MIGRATE MIN MINUS MINUTE_MICROSECOND MINUTE_SECOND MINVALUE MIN_ROWS "
"MOD MODE MODIFIES MODIFY MODULE MORE MOVE MUMPS MUTEX MYSQL_ERRNO "
"NAME NAMES NAMESPACE NATURAL NDB NDBCLUSTER NESTED NESTING NETWORK_NAMESPACE NEVER NEW NEXT NFC NFD NFKC NFKD NIL NO "
...
"PACKAGE PACK_KEYS PAD PAGE PAGE_CHECKSUM PARALLEL PARAMETER PARAMETERS PARAMETER_MODE PARAMETER_NAME "
"PARAMETER_ORDINAL_POSITION PARAMETER_SPECIFIC_CATALOG PARAMETER_SPECIFIC_NAME PARAMETER_SPECIFIC_SCHEMA "
"PARSEONLY PARSER PARSE_GCOL_EXPR PARTIAL PARTITION PARTITIONING PARTITIONS "
"PASCAL PASS PASSING PASSTHROUGH PASSWORD PASSWORD_LOCK_TIME PAST PATTERN "
...
"ROWCOUNT ROWGUIDCOL ROWNUM ROWS ROWTYPE ROW_COUNT ROW_FORMAT ROW_NUMBER RTREE RTRIM RULE RUNNING "
"SAVE SAVEPOINT "
...
"SOURCE_SSL_CA SOURCE_SSL_CAPATH SOURCE_SSL_CERT SOURCE_SSL_CIPHER SOURCE_SSL_CRL SOURCE_SSL_CRLPATH SOURCE_SSL_KEY "
"SOURCE_SSL_VERIFY_SERVER_CERT SOURCE_TLS_CIPHERSUITES SOURCE_TLS_VERSION SOURCE_USER SOURCE_ZSTD_COMPRESSION_LEVEL "
"SPACE SPATIAL SPECIFIC SPECIFICATION SPECIFICTYPE SPECIFIC_NAME SQL SQLCA SQLCODE SQLERROR SQLEXCEPTION SQLSTATE "
"SQLWARNING SQL_AFTER_GTIDS SQL_AFTER_MTS_GAPS SQL_BEFORE_GTIDS SQL_BIG_RESULT SQL_BUFFER_RESULT "
"SQL_CACHE SQL_CALC_FOUND_ROWS SQL_NO_CACHE SQL_SMALL_RESULT SQL_THREAD SQL_TSI_DAY SQL_TSI_FRAC_SECOND SQL_TSI_HOUR "
"SQL_TSI_MINUTE SQL_TSI_MONTH SQL_TSI_QUARTER SQL_TSI_SECOND SQL_TSI_WEEK SQL_TSI_YEAR SQRT SRID SSL STABLE STACKED "
"STANDALONE START STARTING STARTS START_CATALOG START_NAME "
...
"UNICODE UNINSTALL UNION UNIQUE UNKNOWN UNLINK UNLISTEN UNLOCK UNLOGGED UNMATCHED UNNAMED UNNEST UNPIVOT UNREGISTER "
"UNTIL UNTYPED UPDATE UPDATETEXT UPGRADE UPPER URI URL USAGE USE "
"USER USER_DEFINED_TYPE_CATALOG USER_DEFINED_TYPE_CODE USER_DEFINED_TYPE_NAME USER_DEFINED_TYPE_SCHEMA USER_RESOURCES "
...
"CAN_ACCESS_COLUMN( CAN_ACCESS_DATABASE( CAN_ACCESS_TABLE( CAN_ACCESS_USER( CAN_ACCESS_VIEW( CAST( CEIL( CEILING( "
...
"FIELD( FIND_IN_SET( FIRST_VALUE( FLOOR( FORMAT( FORMAT_BYTES( FORMAT_PICO_TIME( FOUND_ROWS( "
...
"INTERNAL_GET_ENABLED_ROLE_JSON( INTERNAL_GET_HOSTNAME( INTERNAL_GET_USERNAME( INTERNAL_GET_VIEW_WARNING_OR_ERROR( "
"INTERNAL_INDEX_COLUMN_CARDINALITY( INTERNAL_INDEX_LENGTH( INTERNAL_IS_MANDATORY_ROLE( INTERNAL_KEYS_DISABLED( "
"INTERNAL_MAX_DATA_LENGTH( INTERNAL_TABLE_ROWS( INTERNAL_UPDATE_TIME( INTERVAL( "
...
"JSON_COMPACT( JSON_CONTAINS( JSON_CONTAINS_PATH( JSON_DEPTH( JSON_DETAILED( "
"JSON_EACH( JSON_EQUALS( JSON_EXISTS( JSON_EXTRACT( JSON_GROUP_ARRAY( JSON_GROUP_OBJECT( JSON_INSERT( JSON_KEYS( "
"JSON_LENGTH( JSON_LOOSE( JSON_MERGE( JSON_MERGE_PATCH( JSON_MERGE_PRESERVE( JSON_NORMALIZE( "
"JSON_OBJECT( JSON_OBJECTAGG( JSON_OVERLAPS( JSON_PATCH( JSON_PRETTY( JSON_QUERY( JSON_QUOTE( JSON_REMOVE( JSON_REPLACE( "
"JSON_SEARCH( JSON_SET( JSON_STORAGE_FREE( JSON_STORAGE_SIZE( JSON_TABLE( JSON_TREE( JSON_TYPE( JSON_UNQUOTE( "
"JSON_VALID( JSON_VALUE( JULIANDAY( "
...
"PS_CURRENT_THREAD_ID( PS_THREAD_ID( Point( PointFromText( PointFromWKB( PointN( PointOnSurface( "
...
"schema( second( session_user( sin( space( sqrt( srid( stddev_pop( stddev_samp( substring( sum( sysdate( system_user( "
...
static const EDITSTYLE Styles_SQL[] = {
	EDITSTYLE_DEFAULT,
	...
	{ MULTI_STYLE(SCE_SQL_STRING_DQ, SCE_SQL_STRING_SQ, 0, 0), NP2StyleX_String, L"fore:#008000" },
...
//Settings++Autogenerated -- start of section automatically generated
		...
		| KeywordAttr32(3, KeywordAttr_NoLexer) // upper case keywords
		| KeywordAttr32(4, KeywordAttr_NoLexer) // upper case data types
		| KeywordAttr32(5, KeywordAttr_NoLexer) // upper case functions
		, SCE_SQL_COMMENTLINEDOC,
		SCE_SQL_STRING_DQ, SCE_SQL_BIT,
//Settings--Autogenerated -- end of section automatically generated
	EDITLEXER_HOLE(L"SQL Query", Keywords_SQL, Styles_SQL),
	L"sql; mysql; hsql",
	Keywords_SQL,
	Styles_SQL
};
