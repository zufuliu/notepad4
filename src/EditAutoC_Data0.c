...
void Style_UpdateLexKeywords(PEDITLEXER pLexNew)
{
	...
	case NP2LEX_GROOVY:
	case NP2LEX_SCALA:
		np2_LexKeyword = &kwJavaDoc;	return;
	case NP2LEX_PHP:
		np2_LexKeyword = &kwPHPDoc;		return;
	case NP2LEX_CSHARP:	// .NET Platform
	case NP2LEX_FSHARP:
	//case NP2LEX_VB:
		np2_LexKeyword = &kwNETDoc;		return;
	case NP2LEX_CPP:
		np2_LexKeyword = &kwDoxyDoc;	return;
	}
	...
	case IDM_LANG_XSLT:
		np2_LexKeyword = &kwXmlStylesheet;	return;
	case IDM_LANG_DTD:
		np2_LexKeyword = &kwXmlDTD;			return;

	case IDM_LANG_ANT_BUILD:
		np2_LexKeyword = &kwAntBuild;		return;
	case IDM_LANG_MAVEN_POM:
		np2_LexKeyword = &kwMavenPOM;		return;
	case IDM_LANG_MAVEN_SETTINGS:
		np2_LexKeyword = &kwMavenSettings;	return;
	case IDM_LANG_IVY_MODULE:
		np2_LexKeyword = &kwIvyModule;		return;
	case IDM_LANG_IVY_SETTINGS:
		np2_LexKeyword = &kwIvySettings;	return;
	case IDM_LANG_PMD_RULESET:
		np2_LexKeyword = &kwPMDRuleset;		return;
	case IDM_LANG_CHECKSTYLE:
		np2_LexKeyword = &kwCheckstyle;		return;

	case IDM_LANG_APACHE:
		np2_LexKeyword = &kwApache;			return;
	case IDM_LANG_TOMCAT:
		np2_LexKeyword = &kwTomcat;			return;
	case IDM_LANG_WEB_JAVA:
		np2_LexKeyword = &kwJavaWebConfig;	return;
	case IDM_LANG_STRUTS:
		np2_LexKeyword = &kwStruts;			return;
	case IDM_LANG_HIB_CFG:
		np2_LexKeyword = &kwHibConfig;		return;
	case IDM_LANG_HIB_MAP:
		np2_LexKeyword = &kwHibMapping;		return;
	case IDM_LANG_SPRING_BEANS:
		np2_LexKeyword = &kwSpringBeans;	return;
	case IDM_LANG_JBOSS:
		return;
